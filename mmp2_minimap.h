...
typedef struct {
	int32_t b, w, k, flag;		// b指定bucket的数目2^b；flag 是众多选项的或，表示该结构体中哪些属性是有值的，哪些属性是无值的。
	...
	struct mm_idx_bucket_s *B; // index (hidden)，动态数组，mm_idx_t在初始化的时候，该数组的capacity就设为2^b。这里的动态数组充当哈希表的作用，
								// 数组的每个元素是一个bucket，每个bucket存储的是minimizer。只是minimizer的哈希值是2*k bits，bucket数目是2^b，
								// 因此只根据哈希值的低b bit确定bucket的位置；然后再用哈希值的剩余的高位bit确定minimizer在bucket内部的值。
	void *km, *h; // km 是内存池
} mm_idx_t; // 表示索引的结构体
...
mm_idx_t *mm_idx_build(const char *fn, int w, int k, int flag, int n_threads, const bwt_t* bwt);
