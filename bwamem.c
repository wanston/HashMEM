 ...
 */
static void mem_collect_intv(const mem_opt_t *opt, const bwt_t *bwt, const mm_idx_t *mi, int len, const uint8_t *seq, smem_aux_t *a)
{
 ...
 * @return
 */
mem_chain_v mem_chain(const mem_opt_t *opt, const bwt_t *bwt, const bntseq_t *bns, const mm_idx_t* mi, int len, const uint8_t *seq, void *buf)
{
	int i, b, e, l_rep;
	...
	mem_collect_intv(opt, bwt, mi, len, seq, aux);
 ...
 * @return
 */
mem_alnreg_v mem_align1_core(const mem_opt_t *opt, const bwt_t *bwt, const bntseq_t *bns, const uint8_t *pac, const mm_idx_t *mi, int l_seq, char *seq, void *buf)
{
	int i;
	mem_chain_v chn;
	mem_alnreg_v regs;
	...
	chn = mem_chain(opt, bwt, bns, mi, l_seq, (uint8_t*)seq, buf);
...
//    fputs(buf, mem_files[pro_tid]);
	...
	if (!(w->opt->flag&MEM_F_PE)) { // single_end
		...
		w->regs[i] = mem_align1_core(w->opt, w->bwt, w->bns, w->pac, w->mi, w->seqs[i].l_seq, w->seqs[i].seq, w->aux[tid]);
	} else { // pair_end
		if (bwa_verbose >= 4) printf("=====> Processing read '%s'/1 <=====\n", w->seqs[i<<1|0].name);
		w->regs[i<<1|0] = mem_align1_core(w->opt, w->bwt, w->bns, w->pac, w->mi, w->seqs[i<<1|0].l_seq, w->seqs[i<<1|0].seq, w->aux[tid]);// 对一条read执行aln
 ...
 * @param pes0
 */
void mem_process_seqs(const mem_opt_t *opt, const bwt_t *bwt, const bntseq_t *bns, const uint8_t *pac, const mm_idx_t* mi, int64_t n_processed, int n, bseq1_t *seqs, const mem_pestat_t *pes0)
{
	...
	w.opt = opt; w.bwt = bwt; w.bns = bns; w.pac = pac; w.mi = mi;
